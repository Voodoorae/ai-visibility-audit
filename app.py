...
st.markdown("""
    ...
    .pain-card {
        ...
        padding: 20px;
        border-radius: 10px;
        border-top: 3px solid #FF4B4B; /* Red accent for pain */
        height: 100%;
        margin-bottom: 20px;
    }
    .pain-title {
    ...
    .score-label { font-size: 16px; text-transform: uppercase; letter-spacing: 3px; color: #8899A6; font-family: 'Inter', sans-serif; }
    .verdict-text { font-size: 24px; font-weight: 800; margin-top: 15px; font-family: 'Spectral', serif; }
...
def fallback_analysis(url):
    ...
    # Dynamic Score (30-55 range)
    score = 30 + (domain_hash % 25)
    
    breakdown = {}
    # Generate breakdown data for PDF (hidden in UI)
...
st.markdown("<div class='sub-head'>Is your business visible to GBP, ABC, Siri, Alexa, and AI Search Agents?</div>", unsafe_allow_html=True)
...
with st.form(key='audit_form'):
    url = st.text_input("", placeholder="Enter URL", key="url_field")
    submit = st.form_submit_button(label='RUN THE AUDIT')

# --- PAIN ROW & 8 SIGNALS SECTION (Only shown before Audit) ---
...
if st.session_state.audit_data:
    ...
    <div class="score-container" style="border-top: 5px solid {score_color};">
        ...
        <div class="score-circle" style="color: {score_color};">{data['score']}/100</div>
        <div class="verdict-text" style="color: {score_color};">{data['verdict']}</div>
    </div>
    """, unsafe_allow_html=True)

    # 2. BLOCKED WARNING (If Applicable)
    ...
    # 3. EMAIL FORM (Lead Magnet) - Direct after Score
    st.markdown("### 1. Get Your Full Report")
    st.markdown("<p style='color:#ccc; font-size:14px;'>Unlock the detailed PDF breakdown including 'How AI Agents See Your Business'.</p>", unsafe_allow_html=True)
    ...
    # 4. TOOLKIT OFFER (Tripwire) - Compact Space
    st.markdown("---")
    st.markdown("<h3 style='text-align: center; color: #FFDA47; margin-bottom: 10px; margin-top: 10px;'>UNLOCK YOUR BUSINESS IN 2-3 HOURS</h3>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #fff; margin-bottom: 20px;'>You are missing critical AI signals. Get the <strong>Fast Fix Toolkit</strong> to unlock your visibility.</p>", unsafe_allow_html=True)
    
    st.markdown("""
    <div style='background-color: #2D3342; padding: 20px; border-radius: 8px; margin-bottom: 30px;'>
    ...
    with b_col2:
        ...
        <a href="https://your-checkout-link-tuneup.com" target="_blank" class="amber-btn">
            BOOK done for you TUNE-UP (£150)
        </a>
        """, unsafe_allow_html=True)

    # 6. START NEW AUDIT (Reset Button)
