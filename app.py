...
# --- ANALYSIS ENGINE (RECALIBRATED THRESHOLDS) ---
def analyze_website(raw_url):
    try:
        ...
        # NEW SCORING RULES: 0-50 RED, 51-80 AMBER, 81+ GREEN
        if final_score <= 50:
...
if st.session_state.audit_data:
    ...
    for i, (k, v) in enumerate(data['breakdown'].items()):
        color = "#28A745" if v['points'] == v['max'] else "#FF4B4B"
        card = f'<div class="audit-card" style="border-left: 5px solid {color};"><h4>{k}</h4><p>{v["note"]}</p></div>'
        if i % 2 == 0: b_col1.markdown(card, unsafe_allow_html=True)
        else: b_col2.markdown(card, unsafe_allow_html=True)

    st.button("🔄 SCAN ANOTHER SITE", on_click=lambda: st.session_state.update({"audit_data": None}), use_container_width=True)
