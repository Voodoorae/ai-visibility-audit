...
def analyze_website(raw_url):
    ...
    except: return {"score": 45, "verdict": "INVISIBLE TO AI", "color": "#FF4B4B", "breakdown": {}}
...
if not st.session_state.audit_data:
    ...
    if submit and url_input:
        for i, p in enumerate(placeholders):
            with p.container(border=True):
                st.markdown(f"### {p_names[i]}"); st.markdown("Status: <span style='color:#FFDA47;'>**🔍 SCANNING...**</span>", unsafe_allow_html=True)
            time.sleep(0.4)
            with p.container(border=True):
                st.markdown(f"### {p_names[i]}"); st.markdown("Status: <span style='color:#28A745;'>**✅ CHECKED**</span>", unsafe_allow_html=True)
        st.session_state.audit_data = analyze_website(url_input); st.session_state.url_input = url_input
...
# --- PROFESSIONAL FOOTER ---
st.markdown("---")
st.markdown('<div style="text-align: center; font-size: 12px; color: #888888; padding-bottom: 20px;">© 2026 Found By AI. Property of Found By AI. <a href="mailto:hello@becomefoundbyai.com" style="color: #FFDA47;">hello@becomefoundbyai.com</a></div>', unsafe_allow_html=True)
